...
The backend may require keys for external data providers and optional AI integrations, depending on which parts of the pipeline are enabled.

Important notes
• Housing mode currently reuses the same parcel dataset as investment mode and applies housing-specific scoring on top.
• If AI billing or quota is unavailable, the pipeline falls back to default insight outputs.
• Some external APIs such as Overpass may occasionally time out; the pipeline is designed to continue where possible.
• Frontend values only match backend files exactly if the transformation layer preserves the raw backend fields.

Suggested next improvements
• add a dedicated housing output file if a separate housing dataset is needed
• reduce frontend recomputation by using backend scores directly where appropriate
• improve neighborhood matching for names such as LAKE vs LAKE VIEW
• add caching and retry improvements for slower external APIs
• add tests for score consistency between backend output and frontend rendering
...
Foresight is a parcel intelligence system that turns raw Chicago property and market data into forecasted, scored, and map-ready investment insights. It is designed to support both broader investment discovery and housing-focused analysis in a single workflow.
