...
export default function IntelPanel({
  ...
  // FIX: When a property is selected, use THAT PROPERTY's metrics for the 6 scores,
  // not the neighborhood's. Each property has its own metrics in the backend data.
  // We pass the prop (which carries _raw with metrics) to getTimelineAwareScoreSet
  // by building a temporary hood-like object with the property's own metrics.
  const scoreSource = prop
    ? { ...hood, metrics: (prop as any).metrics ?? (prop as any)._raw?.metrics ?? (hood as any).metrics, scores: (prop as any)._scores ?? hood.scores }
    : hood;
  const timelineAwareHoodScores = getTimelineAwareScoreSet(scoreSource as Neighborhood, timeline);
