...
import { Neighborhood, Property, ScenarioInputs } from "@/data/types";
...
export default function ScenarioLab({ open, onClose, selectionType, hoodId, propertyId, onNotify }: ScenarioLabProps) {
  ...
  return (
    <div className="fixed inset-0 z-[2000] flex items-center justify-center"
      ...
      <div className="w-[720px] max-w-[92vw] max-h-[85vh] overflow-y-auto rounded-[16px] p-7 relative custom-scroll"
        ...
        <h2 className="text-[20px] font-bold">Scenario Lab · {name}</h2>
        <p className="text-[12px] text-t-muted mb-5">Model acquisition, hold, and exit scenarios with adjustable assumptions</p>
          ...
          {[1, 3, 5].map(y => (
            <div key={y}
              className={`rounded-f p-[14px] text-center cursor-pointer transition-all border ${
                holdYears === y ? "border-f-blue/40 bg-f-blue/10" : "border-white/[0.04] bg-white/[0.02] hover:border-white/[0.08]"
              }`}
        ...
        <div className="grid grid-cols-2 gap-[10px] mb-5">
          {[
            ...
            <div key={item.key} className="rounded-f p-3 border border-white/[0.04] bg-white/[0.02]">
              <label className="text-[10px] font-semibold text-t-muted uppercase tracking-[0.5px] block mb-[6px]">{item.label}</label>
              <input type="text" defaultValue={item.value}
                className="w-full h-8 px-[10px] rounded-[6px] text-[12px] font-sans text-t-primary outline-none"
        ...
        <div className="rounded-f p-4 border border-white/[0.04] bg-white/[0.02] mb-4">
          <h3 className="text-[12px] font-bold uppercase tracking-[0.5px] text-t-secondary mb-3">Projected Returns · {holdYears}-Year Hold</h3>
          {[
            ...
            <div key={row.label} className="flex justify-between py-[6px] border-b border-white/[0.02] last:border-none">
              <span className="text-[12px] text-t-muted">{row.label}</span>
              <span className="text-[13px] font-semibold font-mono" style={{ color: row.color || "#eaf0fa" }}>{row.value}</span>
            </div>
          ))}
        </div>

        <div className="flex gap-2">
          <button onClick={() => { onNotify("Scenario saved to workspace"); onClose(); }}
